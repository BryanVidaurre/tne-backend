...
import { toIsoDateTime } from '../../common/date.util';
...
export type AsistenteRow = {
  ...
  fecha?: string | null; // datetime iso
};

export function parseAsistentes(buffer: Buffer): AsistenteRow[] {
  const wb = XLSX.read(buffer, { type: 'buffer' });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const rows: any[] = XLSX.utils.sheet_to_json(ws, { defval: null });

  const out: AsistenteRow[] = [];
  for (const r of rows) {
    ...
    out.push({
      ...
      medio_ingreso: String(r['Medio Ingreso'] ?? '').trim() || null,
      fecha: toIsoDateTime(r['Fecha']),
    });
  }
  return out;
}
